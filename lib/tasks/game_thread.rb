  ...
  def call(client)
    ...
    games.each_with_index do |game, i|
      ...
      if game_time.year == today.year && game_time.month == today.month && game_time.day == today.day
        ...
        detail_url = "http://www.nba.com/games/#{game_time.year}#{game_time.month}#{game_time.day}/#{game["away_team"][1]}#{game["home_team"][1]}/gameinfo.html"
        ...
        body = [
          ...
          "|**Tip-off time:**|#{game_time.strftime("%I:%M%p")} Eastern (#{(game_time - 3600).strftime("%I:%M%p")} Central, #{(game_time - 7200).strftime("%I:%M%p")} Mountain, #{(game_time - 10800).strftime("%I:%M%p")} Pacific, #{(game_time + 28800).strftime("%I:%M%p")} GMT)|",
          "|**TV/Radio:**|#{media.join(" / ")}|",
